...
(defn- render-recommendations [recs person]
  (list [:h2#anbefalinger (str (:genitive person) " anbefalinger")]
...
(defn- render-hobbies [hobbies _]
  (list [:h2#snakker-om "Snakker gjerne om"]
...
(defn- render-side-projects [side-projects _]
  (list [:h2#sideprosjekter "Sideprosjekter"]
...
(defn- render-presentations [presentations person]
  (list [:h2#foredrag (str (:genitive person) " foredrag")]
...
(defn- render-endorsements [endorsements person]
  (list [:h2#referanser (str (:genitive person) " referanser")]
...
(defn- render-blog-posts [posts person]
  (list
   [:h2#bloggposter (str (:genitive person) " bloggposter")]
...
(defn- render-upcoming [events person]
  (let [date (time/today)
    ...
    (when (seq upcoming)
      (list [:h2#kommende (str (:genitive person) " kommende foredrag/kurs")]
            (->> upcoming
                 (sort-by :date)
