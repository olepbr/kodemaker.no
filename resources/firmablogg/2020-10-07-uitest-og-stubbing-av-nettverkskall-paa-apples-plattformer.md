...
UI-testing har fått dårlig omtale hvor de kritiseres for å være trege, de tar lang tid å skrive, de er ofte ikke spesielt lesbare, brukergrensesnittet endrer seg kontinuerlig og sist men ikke minst så er inkonsistente testdata og mangel på kontroll over responsen fra server en stor utfordring. 
...
På Apple sine plattformer, i rammeverket Foundation så finner man URLSession og relaterte klasser som gir ett API for å laste opp og ned data. En av disse klassene er URLProtocol, som bl.a har denne klassefunksjonen:
...
Her er ett eksempel som viser hvordan man kan gjøre akkurat dette:
...
1. Opprette ett statisk dictionary hvor URL er `key`, og response er `value`.
...
Nedenfor ser vi ett eksempel der vi ønsker å teste en tjeneste som utfører to nettverkskall for å løse oppgaven sin. Dette består av å først hente en public key, for deretter sette en kode hvor denne public key benyttes. Det er altså først ett `GET` kall og deretter en `POST`. 
...
Det neste eksempelet viser en service som gjør ett nettverkskall, hvor man verifiserer at resultatet inneholder forventede verdier.
...
Det finner nok mange andre kreative og bedre fremgangsmåter enn dette man kan velge, og ett alternativt er at man bestemmer man seg for at det er så mye arbeid at det ikke er verdt det. Jeg håper  det ikke er tilfellet etter du har lest denne bloggposten :)
...
I AppDelegate så la jeg inn følgende kodesnutt, som sjekker om appen kjøres under en UI-test, og gjør man det, så kalles `executeUITestWithStubs`. Jeg har lagt denne sjekken inne i en `#if DEBUG` conditional compile block, som betyr at denne koden vil ikke komme med i Release bygget. For å sjekke om vi kjører under en UI-test så sjekker jeg om argumentet `--UITests` er satt i launchArguments på UI-test siden. Vi vil se dette senere. 
...
3. Pga det litt naive dataformatet som er benyttet, så er maxSplits satt til 2 for ikke å splitte på selve inneholdet i data.
